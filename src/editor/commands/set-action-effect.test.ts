...
describe("setActionEffect command (D-54)", () => {
  ...
  it.each(fixtures)("apply→invert→apply is idempotent: $name", async ({ args }) => {
    ...
    const originalEffect = before.spec.actions[args.id];
    expect(originalEffect).toBeDefined();

    const { spec: after1, inverseArgs } = setActionEffect.apply(before.spec, before.astHandle, args);
    ...
    expect(restored.actions[args.id]?.kind).toBe(originalEffect?.kind);
