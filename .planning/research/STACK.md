...
**Domain:** pi.dev TypeScript extension — rich TUI for authoring/editing/exporting mobile-app spec files (Markdown + YAML frontmatter, ASCII wireframes, Maestro/Detox YAML)
**Researched:** 2026-04-17
**Confidence:** HIGH for pi.dev runtime, TUI toolkit, YAML/Markdown, Maestro, Zod; MEDIUM for Detox-emission ergonomics; LOW for ASCII wireframe libraries (none fit — recommendation is "build our own on top of `pi-tui`").

---

## Executive Take

The stack is heavily opinionated by one decision: **we are a pi.dev TypeScript extension, not a standalone CLI.** That single constraint eliminates ~80% of the usual Node TUI decision space:
...
Core value is "**wireframe-first spec**." Three pieces of the stack carry real weight:

1. **`pi-tui` primitives** (`Text`, `Box`, `Container`, `SelectList`, `Editor`, plus custom `Component`s we write) — the wizard + canvas + wireframe preview all compose from these.
2. **`gray-matter` + `yaml` (eemeli)** for the spec file's front-matter/body split, with `yaml` chosen over `js-yaml` specifically because the spec file is edited by humans in git, and we want comment-and-blank-line round-trip.
3. **`zod@4`** as the single schema definition — we derive TypeScript types, validate at every read, and serialize error messages into TUI toasts via `ctx.ui.notify()`.

Everything else (Maestro YAML, Detox artifact emission, ASCII wireframe rendering) is a thin, domain-specific layer on top.
...
npm install -D @mariozechner/pi-coding-agent @mariozechner/pi-tui @mariozechner/pi-ai

# Dev tooling
npm install -D typescript@^5.6 tsup@^8.5 @biomejs/biome@^2.4 vitest@^4.1 @vitest/coverage-v8@^4.1
```
...
```json
{
  ...
  "scripts": {
    "build": "tsup src/index.ts --format esm --dts --clean --external @mariozechner/pi-coding-agent --external @mariozechner/pi-tui --external @mariozechner/pi-ai",
...
Users install via `pi install npm:mobile-tui` or `pi install git:github.com/USER/mobile-tui`.

---

## pi.dev Extension Shape (concrete)

This is the piece no generic "TUI stack" research would surface. Locking it down up front prevents an architecture mistake that would cascade into every phase.
...
export default function (pi: ExtensionAPI) {
  // 1. Slash command = entrypoint (users type `/spec` in pi)
  pi.registerCommand("spec", {
    description: "Open the mobile app spec wizard / canvas",
    handler: async (args, ctx) => {
      // 2. Open persistent custom UI — takes over the editor slot
      await ctx.ui.custom(
        (tui, theme, keybindings, done) => {
          // Return our root Component (wizard or canvas)
          return new SpecRootComponent({ theme, keybindings, onExit: done });
        },
        { overlay: false } // full TUI mode, not overlay
      );
    },
  });

  // 3. Optional keyboard shortcut from anywhere in pi
  pi.registerShortcut("ctrl+shift+s", {
    description: "Open mobile spec canvas",
    handler: async (ctx) => { /* invoke same flow */ },
  });

  // 4. Optional: expose LLM-callable tools so the pi agent can
  //    read/write spec sections on behalf of the user
  //    pi.registerTool({ name: "spec_get_screen", parameters: Type.Object({...}), ... })
}
```

**Key patterns enforced by pi:**
...
**Recommendation: do not pull in a library. Build a small wireframe layout engine on top of `pi-tui`'s `Component`.**
...
A `Wireframe` module that emits a `Component`. Takes a structured description:

```ts
type WireframeNode =
  | { kind: "screen"; title: string; statusBar?: boolean; children: WireframeNode[] }
  | { kind: "navbar"; left?: string; title: string; right?: string }
  | { kind: "list"; items: Array<{ text: string; trailing?: string }> }
  | { kind: "card"; title?: string; body: string[] }
  | { kind: "button"; label: string; style?: "primary" | "secondary" }
  | { kind: "input"; label: string; placeholder?: string }
  | { kind: "tabbar"; items: string[]; active: number }
...
This fits exactly within `pi-tui`'s component model — the wireframe *is* a `Component` the canvas preview pane renders like any other child.
...
No library exists for "generate Maestro YAML from structured input" — and we don't need one. Our Zod-validated spec AST maps 1:1 to Maestro commands via a pure function:

```ts
// src/emitters/maestro.ts
import YAML from "yaml";
import type { SpecFlow } from "../schema";

export function emitMaestroFlow(flow: SpecFlow, appId: string): string {
  const config = { appId, tags: flow.tags ?? [] };
  const steps = flow.steps.flatMap(stepToMaestroCommands);
  // Frontmatter + --- separator + commands
  return `${YAML.stringify(config)}---\n${YAML.stringify(steps)}`;
}
```

Validation path: emit YAML, then shell out to `maestro check-flow-syntax <file>` in integration tests (behind a `MAESTRO_CLI=1` env flag — skipped if binary missing).

**Optional hardening:** publish a JSON Schema for our spec DSL to SchemaStore, then IDE users get autocomplete in the spec file. The YAML Language Server supports schema discovery. This is a v2 polish item, not a v1 requirement.

### Detox (v2 only)

If/when we add Detox emission, we generate `.e2e.ts` files using a template approach (no library) and assume the target project already has Detox wired up. The Detox emitter consumes the same Zod-typed spec AST — adapter pattern, not re-designing the schema.
...
**If we expose LLM-callable tools (e.g., "agent, read screen X's wireframe"):**
...
**If a user hits us with a large spec (100+ screens) and render perf matters:**
...
**If we ever want to share the spec format with non-pi consumers (e.g., a VS Code extension):**
...
**If comment preservation in YAML proves unreliable for our DSL shape:**
...
These came out of reading the pi extension docs carefully. Every one of them will bite in a specific phase if we don't call it out now.

1. **Extensions load via jiti, not `tsc`.** TypeScript errors at import time surface as runtime load failures. Keep `tsc --noEmit` in CI and pre-commit.
2. **Runtime deps must be in `dependencies`, not `devDependencies`.** pi installs published packages with `--omit=dev`. If `gray-matter` or `yaml` sneaks into devDeps, the extension crashes on `pi install npm:...`.
3. **Peer-dep the pi packages, do not bundle them.** Bundling `@mariozechner/pi-tui` into our dist ships a second copy of pi-tui at runtime, breaks `instanceof` checks and the shared theme registry.
4. **`ctx.hasUI === false` in `-p` and `--mode json`.** Guard every `ctx.ui.*` call. For a UI-first tool, the right response in headless mode is `ctx.ui.notify(...)` equivalents → emit a structured error to stderr and exit.
5. **`ctx.ui.custom()` replaces the editor slot.** While our wizard/canvas is active, the user can't type free-form prompts to the pi agent. This is intended — but document it in the `/spec` command's description.
6. **Session state lives on the `SessionManager`, not module-level variables.** If the user runs `/new` or `/fork`, `session_start` fires with `reason: "new" | "fork"` and our in-memory state is stale. Subscribe to `session_start` and rehydrate from disk (the spec file) or from `sessionManager.getEntries()` custom entries.
7. **File mutations must go through `withFileMutationQueue(absPath, fn)`.** If the pi agent or a user-invoked `edit` tool touches the spec file while our canvas is open, we race. The queue serializes writes per-file.
8. **Themes are not imported — they're injected via `ctx.ui.theme` or passed through `Component` constructors.** Hardcoding chalk colors means the user's light theme breaks our UI. Stick to `theme.fg("role", text)` everywhere except wireframe structural glyphs.
9. **Rendering width is variable.** `Component.render(width)` is called with whatever pi's terminal width is. The wireframe has a *fixed* target width (phone chars), but the outer canvas must pad/truncate to fit the terminal. Plan for width mismatch up front — do not assume 80 cols or `process.stdout.columns`.
10. **Overlay mode is "experimental" (per pi docs).** We intend to use full-editor-slot mode (`overlay: false`), which is the primary path. Avoid depending on overlay features for MVP.
11. **No test runner for `ctx.ui.*`.** Unit-test our `Component.render(...)` outputs as string arrays and keep pi-API surface behind a thin adapter. Integration tests need a live pi instance — run them manually for v1.
...
**pi.dev runtime (HIGH confidence):**
...
**Maestro / E2E (HIGH for Maestro, MEDIUM for Detox emission ergonomics):**
...
**Wireframe rendering (MEDIUM — negative finding):**
...
*Stack research for: pi.dev TypeScript extension with rich TUI for mobile app spec authoring*
*Researched: 2026-04-17*
